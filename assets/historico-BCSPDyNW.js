import"./modulepreload-polyfill-Dezn_h7o.js";/* empty css              */import{g as e,m as t,n,r,t as i}from"./utils--YuW_03s.js";document.addEventListener(`DOMContentLoaded`,()=>{let a=document.getElementById(`searchForm`),o=document.getElementById(`telefone`),s=document.getElementById(`resultsContainer`),c=document.getElementById(`resultsList`),l=document.getElementById(`noResults`);o.addEventListener(`input`,e=>{let t=e.target.value.replace(/\D/g,``);t.length>11&&(t=t.slice(0,11)),t.length>6?e.target.value=`(${t.slice(0,2)}) ${t.slice(2,7)}-${t.slice(7)}`:t.length>2?e.target.value=`(${t.slice(0,2)}) ${t.slice(2)}`:t.length>0?e.target.value=`(${t.slice(0,2)}`:e.target.value=``}),a.addEventListener(`submit`,async a=>{a.preventDefault();let u=o.value.replace(/\D/g,``);if(u.length<10){r(`Digite um número de WhatsApp válido.`,`error`);return}try{r(`Buscando seus orçamentos...`,`info`);let[a,o]=await Promise.all([t(),e()]),d=a.filter(e=>{let t=(e.telefone||``).replace(/\D/g,``);return t===u||t.endsWith(u)||u.endsWith(t)});if(d.length===0){s.style.display=`none`,l.style.display=`block`;return}let f=o.filter(e=>d.some(t=>t.id===e.lead_id));c.innerHTML=``,d.forEach(e=>{let t=f.filter(t=>t.lead_id===e.id);if(t.length===0){let t=document.createElement(`div`);t.className=`glass-card`,t.style.marginBottom=`1rem`,t.innerHTML=`
