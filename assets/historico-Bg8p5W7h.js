import{_ as e,a as t,i as n,r,y as i}from"./utils-g9sFxRsc.js";/* empty css              */document.addEventListener(`DOMContentLoaded`,()=>{let a=document.getElementById(`searchForm`),o=document.getElementById(`telefone`),s=document.getElementById(`resultsContainer`),c=document.getElementById(`resultsList`),l=document.getElementById(`noResults`);o.addEventListener(`input`,e=>{let t=e.target.value.replace(/\D/g,``);t.length>11&&(t=t.slice(0,11)),t.length>6?e.target.value=`(${t.slice(0,2)}) ${t.slice(2,7)}-${t.slice(7)}`:t.length>2?e.target.value=`(${t.slice(0,2)}) ${t.slice(2)}`:t.length>0?e.target.value=`(${t.slice(0,2)}`:e.target.value=``}),a.addEventListener(`submit`,async a=>{a.preventDefault();let u=o.value.replace(/\D/g,``);if(u.length<10){t(`Digite um número de WhatsApp válido.`,`error`);return}try{t(`Buscando seus orçamentos...`,`info`);let[a,o]=await Promise.all([e(),i()]),d=a.filter(e=>{let t=(e.telefone||``).replace(/\D/g,``);return t===u||t.endsWith(u)||u.endsWith(t)});if(d.length===0){s.style.display=`none`,l.style.display=`block`;return}let f=o.filter(e=>d.some(t=>t.id===e.lead_id));c.innerHTML=``,d.forEach(e=>{let t=f.filter(t=>t.lead_id===e.id);if(t.length===0){let t=document.createElement(`div`);t.className=`glass-card`,t.style.marginBottom=`1rem`,t.innerHTML=`
          ...
          `,c.appendChild(t)}else t.forEach(t=>{let i=document.createElement(`div`);i.className=`glass-card`,i.style.marginBottom=`1rem`;let a=new Date(t.data_criacao).toLocaleDateString(`pt-BR`),o=t.status===`Fechado`?`var(--status-fechado)`:t.status===`Enviado`?`var(--status-enviado)`:t.status===`Perdido`?`var(--status-perdido)`:`var(--status-novo)`,s=t.status===`Fechado`?`var(--status-fechado-bg)`:t.status===`Enviado`?`var(--status-enviado-bg)`:t.status===`Perdido`?`var(--status-perdido-bg)`:`var(--status-novo-bg)`;i.innerHTML=`
            ...
            `,c.appendChild(i)})}),s.style.display=`block`,l.style.display=`none`}catch(e){console.error(`Erro ao buscar orçamentos:`,e),t(`Erro ao buscar orçamentos. Tente novamente.`,`error`)}})});